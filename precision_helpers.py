...
def mandelbrot_divergence(win_size=1024, max_iters=100, TAU=0.01, x_set=(-0.7530, -0.7490), y_set=(0.0990, 0.1030)):
...
def mandelbrot_sensitivity(win_size=1024, max_iters=100, x_set=(-0.7530, -0.7490), y_set=(0.0990, 0.1030)):
    x = np.linspace(x_set[0], x_set[1], win_size)
    y = np.linspace(y_set[0], y_set[1], win_size)
    C = (x[np.newaxis, :] + 1j * y[:, np.newaxis]).astype(np.complex128)
    eps32 = float(np.finfo(np.float32).eps)
    delta = np.maximum(eps32 * np.abs(C), 1e-10)
    def escape_count(C, max_iters):
...
if __name__ == "__main__":
    ...
    mandelbrot_sensitivity(max_iters=1000)